...
load_dotenv(dotenv_path='envs/lightrag.env', override=False)
...
async def volcengine_openai_complete(
    prompt,
    system_prompt=None,
    history_messages=None,
    enable_cot: bool = False,
...
class LocalModelEmbed:
    def __init__(self, model_dir: str, batch_size=32, normalize_embeddings=True, max_tokens=None):
        ...
        self._embedding_dim: int = self._get_embedding_dim()

    def _truncate(self, text: str):
        if not self._max_tokens: return text
        tokens = self._tokenizer.encode(text, truncation=True, max_length=self._max_tokens)
        return self._tokenizer.decode(tokens)

    async def embed(self, texts: list[str]) -> np.ndarray:
        texts = [self._truncate(t) for t in texts]
        vec = self._model.encode(texts, batch_size=self._batch_size, normalize_embeddings=self._normalize_embeddings)
    ...
    def _get_embedding_dim(self) -> int:
        test_vec = self._model.encode(['test'], batch_size=self._batch_size, normalize_embeddings=self._normalize_embeddings)
