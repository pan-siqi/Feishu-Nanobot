...
**改动量化**：全新文件 ~850 行；小改文件累计 ~200 行；不动文件 0 行风险。
...
飞书用户        feishu.py       loop.py        context.py      memory.py     decision.py
   ...
   │               │               │                               │──store()───────▶│
   │               │               │                               │  写入SQLite     │
   │               │──build_system_prompt(current_msg)─────────────▶               │
...
决策被存入 LightRAG 时以如下文本格式插入，LightRAG 自动抽取实体-关系，写入 Neo4j 或内置 KV 图：
...
这样在 recall 时就可以走 LightRAG 的 `hybrid` 模式（语义向量 + 图关系联合检索），精准度远高于纯向量。
...
**白话**：先用"模糊联想"（向量）找一批，再用"精确对词"（SQL）找一批，两批合并后按时间新旧打分排序，取前 3 名。命中的决策顺手给"记忆强度"充值，让它下次更慢被遗忘。
...
**间隔重复效果**：每次被 recall 命中 → `S *= 1.8`。这意味着一个决策被用了 5 次后，S 已从初始值涨了约 18 倍，要接近 100 天才会跌破遗忘阈值。
...
- 天天被用到的决策（反复 recall）→ S 不断涨 → 衰减变慢 → 永远不会被提醒（用到即是强化）
- 很重要但一直没被提到的决策 → S 不涨 → 约 5 天后跌破 0.4 → cron 发提醒
- 不重要且没被用到的决策 → R 也会跌，但 importance ≤ 0.6 → 直接淡出，不烦人
...
**版本链查询**：查任意历史时可以走 `supersedes` 指针溯源：

```
dec_003（active）─supersedes─▶ dec_001（superseded）─supersedes─▶ dec_000（superseded）
```

**白话**：群里今天说"改用方案 C 了"，系统给上周"用方案 B"那张便利贴打个删除线，但**不丢**——以后还能查"为啥又改了"。降级方案（时间紧时）：跳过 LLM 二次裁决，纯规则版（topic 完全匹配即覆盖），损失约 5% 准确率但 0 LLM 调用成本。
