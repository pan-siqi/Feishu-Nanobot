...
from nanobot.providers.base import LLMProvider, LLMResponse, LLMResponseStructure, ToolCallRequest
...
class OpenAICompatProvider(LLMProvider):
    ...
    def _parse_structured(self, response: Any) -> LLMResponse | LLMResponseStructure:
        """
        Parse result from client.beta.chat.completions.parse(...).

        Expected behavior:
        - Prefer message.parsed when available
        - Fall back to message.refusal or message.content
        - Preserve finish_reason / usage
        """
        if isinstance(response, str):
            return LLMResponse(content=response, finish_reason="stop")
        ...
        if response_map is not None:
            ...
            parsed = msg0.get("parsed")
            refusal = msg0.get("refusal")
            content = self._extract_text_content(msg0.get("content"))

            if parsed is not None and choice0.get("message") is not None:
            ...
            if refusal:
                refusal_text = refusal if isinstance(refusal, str) else str(refusal)
                return LLMResponse(
            ...
            return LLMResponse(
                content="Error: Parsed response had neither parsed object nor content.",
                finish_reason="error",
                usage=self._extract_usage(response_map),
            )
        ...
        if refusal:
            return LLMResponse(
                content=refusal if isinstance(refusal, str) else str(refusal),
                finish_reason=finish_reason,
                usage=self._extract_usage(response),
            )
    ...
    def set_scheme(self, scheme: Type[BaseModel]|Dict):
        self._scheme = scheme

    async def chat_scheme(
        self,
        messages: list[dict[str, Any]],
        tools: list[dict[str, Any]] | None = None,
        model: str | None = None,
        ...
        reasoning_effort: str | None = None,
        tool_choice: str | dict[str, Any] | None = None,
    ) -> LLMResponseStructure | LLMResponse:
        if self._scheme is None:
            raise Exception('call this function after setting scheme')
        try:
            if self._should_use_responses_api(model, reasoning_effort):
                raise NotImplementedError("`chat_scheme` does not support responses API path")

            kwargs = self._build_kwargs(
                messages, tools, model, max_tokens, temperature,
                reasoning_effort, tool_choice,
            )
            kwargs['response_format'] = self._scheme # add scheme option
            # return self._parse(await self._client.chat.completions.create(**kwargs))
            return self._parse_structured(await self._client.beta.chat.completions.parse(**kwargs))
        except Exception as e:
            return self._handle_error(e, spec=self._spec, api_base=self.api_base)
